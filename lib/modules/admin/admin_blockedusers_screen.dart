...
import 'package:carpooling/modules/admin/admin_visit_blocked_profile_screen.dart';
...
class AdminBlockedUsersScreen extends StatelessWidget
{
    ...
    return BlocConsumer<AdminCubit, AdminStates>(
      ...
      builder: (context, state) {
        return ConditionalBuilder(
          condition: AdminCubit.get(context).blockedusers.length > 0,
          builder: (context) => ListView.separated(
            physics: BouncingScrollPhysics(),
            itemBuilder: (context, index) =>
                buildUserItem(AdminCubit.get(context).blockedusers[index], context),
            separatorBuilder: (context, index) => myDivider(),
            itemCount: AdminCubit.get(context).blockedusers.length,
          ),
          fallback: (context) => Center(child: Text('No Blocked Users',style: TextStyle(fontSize: 40.0,color: Colors.grey),)),
        );
      },
    );
  }
  ...
  Widget buildUserItem(CarpoolingUserModel model, context) => InkWell(
    onTap: () {
      ...
      navigateTo(context, AdminVisitBlockedProfileScreen());
    },
    child: Padding(
